...
def home(request):
  ...
  for soba in rooms:
    upcoming_bookings = Bookings.objects.filter(room=soba, start_time__gte=current_time, start_time__lt=current_time + timezone.timedelta(days=8)).order_by('start_time') # samo 14 dana unapred
